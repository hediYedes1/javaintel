...
public class Level2 {
    ...
    public static void main(String[] args) {
        ...
        long nbr = teachers.stream().filter(te ->te.getName().startsWith("S")).count();

        /* TO DO 2: Retourner la somme des salaires de tous les enseignants Flutter (hint: mapToInt) */
        long sum = teachers.stream().filter(te ->te.getSubject().equals(Subject.FLUTTER)).mapToInt(Teacher::getSalary)
                .sum();

        /* TO DO 3: Retourner la moyenne des salaires des enseignants dont le nom commence avec a */
        double average = teachers.stream().filter(teacher -> teacher.getName().startsWith("a")).mapToInt(Teacher::getSalary)
                .average().orElse(0);


        /* TO DO 4: Retourner la liste des enseignants dont le nom commence par f */
        List<Teacher> teachers1 = teachers.stream().filter(te ->te.getName().startsWith("f"))
        ...
        /*First way*/
        teachers.stream().filter(teacher -> teacher.getName().startsWith("g") && teacher.getSubject().equals(Subject.UNITY))
        ...
        /*Second way*/
        teachers.stream()
                .filter(te -> te.getSubject().equals(Subject.UNITY) && te.getName().startsWith("G"))
                .findFirst()
                .ifPresent(enseignant -> System.out.println(enseignant));

        /* TO DO 7: Afficher le deuxième enseignant dont le nom commence avec s */
        teachers.stream().filter(te ->te.getName().startsWith("S"))
